...
const BoardPage = () => {
  ...
  const [draggedOverPosition, setDraggedOverPosition] = useState<{listId: string, index: number} | null>(null);
  ...
  const handleCardDragOver = (e: React.DragEvent, listId: string, cardIndex: number) => {
    ...
    const insertIndex = y < cardMiddle ? cardIndex : cardIndex + 1;
    
    setDraggedOverPosition({ listId, index: insertIndex });
    setDraggedOverList(listId);
  };

  const handleEmptyListDragOver = (e: React.DragEvent, listId: string) => {
    e.preventDefault();
    e.stopPropagation();
    setDraggedOverPosition({ listId, index: 0 });
    setDraggedOverList(listId);
  };
  ...
  const handleDrop = (e: React.DragEvent, targetListId: string) => {
    ...
    if (!draggedCard || !draggedOverPosition) {
      setDraggedCard(null);
      setDraggedOverList(null);
      setDraggedOverPosition(null);
      return;
    ...
    const { listId: dropListId, index: dropIndex } = draggedOverPosition;
    
    setBoard(prevBoard => {
      const newLists = prevBoard.lists.map(list => {
        if (list.id === sourceListId) {
          // Remove card from source list
          const newCards = list.cards.filter((_, index) => index !== sourceIndex);
      ...
      const updatedLists = newLists.map(list => {
        if (list.id === dropListId) {
          ...
          // Adjust drop index if moving within the same list
          let finalDropIndex = dropIndex;
          if (sourceListId === dropListId && sourceIndex < dropIndex) {
            finalDropIndex = dropIndex - 1;
          }
          
          newCards.splice(finalDropIndex, 0, card);
          return { ...list, cards: newCards };
        }
        return list;
      });
  ...
  const getDropIndicatorPosition = (listId: string, cardIndex: number) => {
    if (!draggedOverPosition || draggedOverPosition.listId !== listId) {
      return null;
    }
    
    return draggedOverPosition.index === cardIndex ? 'before' : 
           draggedOverPosition.index === cardIndex + 1 ? 'after' : null;
  };
          ...
          {board.lists.map((list) => (
            ...
            >
                ...
                <div className="flex items-center justify-between w-full text-black/80">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-gray-800 text-white border border-gray-600 shadow-xl rounded-md w-32 p-1">
                        <DropdownMenuItem
                          onClick={() => {
                          ...
                          className="group flex items-center gap-2 px-3 py-2 rounded-md text-sm text-red-500 hover:bg-red-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-red-500 transition-all"
                        >
              ...
              <div className="p-3 space-y-2 min-h-[200px]" 
                   onDragOver={list.cards.length === 0 ? (e) => handleEmptyListDragOver(e, list.id) : undefined}>
                {/* Drop indicator for empty list */}
                {list.cards.length === 0 && draggedOverPosition?.listId === list.id && (
                  <div className="h-0.5 bg-blue-400 rounded-full mb-2"></div>
                )}
                ...
                {list.cards.map((card, cardIndex) => (
                  <div key={card.id} className="relative">
                    ...
                    {getDropIndicatorPosition(list.id, cardIndex) === 'before' && (
                      <div className="h-0.5 bg-blue-400 rounded-full mb-2"></div>
                    )}
                    ...
                    <Card 
                      className={`cursor-move hover:shadow-lg transition-all duration-200
                                ...
                                ${draggedCard?.card.id === card.id ? 'opacity-50 rotate-3 scale-105' : ''}`}
                      draggable
                      onDragStart={(e) => handleDragStart(e, card, list.id, cardIndex)}
                      onDragEnd={handleDragEnd}
                      onDragOver={(e) => handleCardDragOver(e, list.id, cardIndex)}
                      onClick={() => {
                    ...
                    {getDropIndicatorPosition(list.id, cardIndex) === 'after' && (
                      <div className="h-0.5 bg-blue-400 rounded-full mt-2"></div>
                    )}
                  </div>
                ))}

                {/* Drop indicator at the end of list */}
                {draggedOverPosition?.listId === list.id && 
                 draggedOverPosition.index === list.cards.length && (
                  <div className="h-0.5 bg-blue-400 rounded-full mt-2"></div>
                )}
